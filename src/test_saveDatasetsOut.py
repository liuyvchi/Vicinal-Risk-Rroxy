...
def main(args):
    ...
    dataset_name, hard_level = args.ImageNetV2_path.split('/')[-2], args.ImageNetV2_path.split('/')[-1]

    mixup_prediction_path1 = '/root/paddlejob/workspace/env_run/afs/liuyuchi/autoEval/modelOutput/%s/%s.npy'%(model_name+'_mixup', dataset_name+'_'+hard_level)
    grey_prediction_path1 = '/root/paddlejob/workspace/env_run/afs/liuyuchi/autoEval/modelOutput/%s/%s.npy'%(model_name+'_grey', dataset_name+'_'+hard_level)
    rotation_prediction_path1 = '/root/paddlejob/workspace/env_run/afs/liuyuchi/autoEval/modelOutput/%s/%s.npy'%(model_name+'_rotation', dataset_name+'_'+hard_level)

    if not os.path.exists(mixup_prediction_path1):
        pass
    else:
    ...
    eval_transforms = transforms.Compose([transforms.ToPILImage(), transforms.Resize((h,w)), transforms.ToTensor(), transforms.Normalize(mean, std)])
    ...
    # wjscore_transforms = transforms.Compose([transforms.ToPILImage(), transforms.Resize((h,w)), transforms.ToTensor(), MyRotationTransform(angles=[-90, 90, 180]), transforms.Normalize(mean, std)])
    ...
    log_dir = os.path.join("/root/paddlejob/workspace/env_run/output/", args.model_name+'_grey')
    ...
    results_dic = get_mixup_predictions(model, wj_test_loader, device)
    print("--- %s seconds ---" % (time.time() - start_time))
    print(results_dic['acc'])

    np.save(os.path.join(log_dir ,dataset_name+'_'+hard_level+'.npy'), results_dic)
