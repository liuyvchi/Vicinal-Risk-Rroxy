# -*- coding: utf-8 -*-
##### this code is for getting the statistics of various risk for visualization and analysis
...
def test_wj(test_loader, exter_knowledge_dic, device):
    ...
    with torch.no_grad():
        ...
        ## 用于存放所有样本的risk和对错与否
        total_sample_num = length
        risk_pool_erp = {}
        risk_pool_vrp = {}
        risk_pool_erp['EI'] = np.zeros(total_sample_num)
        risk_pool_vrp['EI'] = np.zeros(total_sample_num)
        risk_pool_erp['AC'] = np.zeros(total_sample_num)
        risk_pool_vrp['AC'] = np.zeros(total_sample_num)
        risk_pool_erp['CI'] = np.zeros(total_sample_num)
        risk_pool_vrp['CI'] = np.zeros(total_sample_num)
        risk_pool_erp['DoC'] = np.zeros(total_sample_num)
        risk_pool_vrp['DoC'] = np.zeros(total_sample_num)
        risk_pool_erp['ATC'] = np.zeros(total_sample_num)
        risk_pool_vrp['ATC'] = np.zeros(total_sample_num)

        correctness_pool = np.zeros(total_sample_num)
        ...
        for batch_i, (prediction1_1, prediction1_2, prediction2_1, prediction2_2, acc, gt, idx) in enumerate(test_loader):
            ...
            ## 修改对错pool
            correctness_pool[idx[batch_correct_index].cpu().numpy()]=1
            ## EI
            risk_pool_erp['EI'][idx.cpu().numpy()] = torch.mul(confidence1, confidence2).sqrt().mul(same_pred.long()).cpu().numpy()
            risk_pool_vrp['EI'][idx.cpu().numpy()] = EI_randomPair.sum(dim=-1).div(s22_gk.mul(labelM2).sum(-1)).cpu().numpy()
            # AC
            risk_pool_erp['AC'][idx.cpu().numpy()] = confidence1.cpu().numpy()
            risk_pool_vrp['AC'][idx.cpu().numpy()] = AC_vrp.sum(dim=-1).div(s22_gk.mul(labelM2).sum(-1)).cpu().numpy()
            #CI
            risk_pool_erp['CI'][idx.cpu().numpy()] = (1-p21).cpu().numpy()
            risk_pool_vrp['CI'][idx.cpu().numpy()] = confidence_Inv_vrp.sum(dim=-1).div(s22_gk.mul(labelM2).sum(-1)).cpu().numpy()

            #DoC
            risk_pool_erp['DoC'][idx.cpu().numpy()] = (S_accuracy - (S_AC - confidence1)).cpu().numpy()
            risk_pool_vrp['DoC'][idx.cpu().numpy()] = (S_accuracy - (S_AC - AC_vrp.sum(dim=-1).div(s22_gk.mul(labelM2).sum(-1)))).cpu().numpy()
            #ATC
            risk_pool_erp['ATC'][idx.cpu().numpy()] = (confidence1>S_ATC_t).cpu().numpy()
            risk_pool_vrp['ATC'][idx.cpu().numpy()] = ATC_vrp_acc.sum(dim=-1).div(s22_gk.mul(labelM2).sum(-1)).cpu().numpy()
     
    return correctness_pool, risk_pool_erp, risk_pool_vrp
...
def main(args):
    ...
    grey_prediction_path1 = '/root/paddlejob/workspace/env_run/afs/liuyuchi/autoEval/modelOutput/objectnet_out_rotation/%s.npy'%(model_name)
    grey_prediction_path2 = '/root/paddlejob/workspace/env_run/afs/liuyuchi/autoEval/modelOutput/objectnet_out_rotation/%s.npy'%(model2_name)
    ...
    log_dir = os.path.join("/root/paddlejob/workspace/env_run/output/", "objectnet_out_rotation_sampleRisk")
    ...
    correctness_pool, risk_pool_erp, risk_pool_vrp = test_wj(wj_test_loader, exter_knowledge_dic, device)
    print("--- %s seconds ---" % (time.time() - start_time))

    dic = {}
    dic['correctness_pool'] = correctness_pool
    dic['risk_pool_erp'] = risk_pool_erp
    dic['risk_pool_vrp'] = risk_pool_vrp
